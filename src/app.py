...
class MainWindow(wx.Frame):
  ...
  def __init__(self, parent, id, title):
    ...
    feed_txt = wx.StaticText(
        self, -1, "Text to be used to fill the image (optional) : ")
    ...
    width_txt = wx.StaticText(
        self, -1, "Width (Integer. Default is %d) : " % DEFAULT_RES_IMG_WIDTH)
    ...
    wx.EVT_BUTTON(self, ID_BUTTON_OPEN_FEED,
                  self.OnCustmOpen(self.feed_control))
  ...
  def OnAbout(self, e):
    d = wx.MessageDialog(
        self, "Choose a image file and and save it as a colorful HTML file filled with custom text!", "About", wx.OK)
  ...
  def OnSave(self, e):
    show_success = False
    msg_txt, msg_title = 'Unknown', 'Buggy code'
    dlg = wx.FileDialog(self, "Save As", self.dirname, "", "*.html", wx.FD_SAVE)
    if dlg.ShowModal() == wx.ID_OK:
      output_file = dlg.GetPath()
      if(os.path.isfile(output_file)):
        overwrite = wx.MessageDialog(
      ...
      converter = ImgToHTML(self.img_control.GetValue(),
                            self.feed_control.GetValue())
